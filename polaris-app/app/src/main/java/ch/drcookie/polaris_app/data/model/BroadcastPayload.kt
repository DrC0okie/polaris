...
data class BroadcastPayload(
    ...
    init {
        ...
        require(signature.size == PoLConstants.SIG_SIZE) {
            "Invalid signature size. Expected ${PoLConstants.SIG_SIZE}, got ${signature.size}"
        }
    }

    companion object {
        // Total size of the payload: 4 (id) + 8 (counter) + 64 (sig) = 76 bytes
        const val PACKED_SIZE = 4 + 8 + PoLConstants.SIG_SIZE
        ...
        fun fromBytes(data: UByteArray): BroadcastPayload? {
            ...
            val signature = data.sliceArray(offset until offset + PoLConstants.SIG_SIZE)
