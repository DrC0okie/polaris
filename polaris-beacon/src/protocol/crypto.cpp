...
void generateEd25519KeyPair(uint8_t public_key_out[POL_Ed25519_PK_SIZE],
    ...
    if (crypto_sign_ed25519_keypair(public_key_out, secret_key_out) != 0) {
        ...
        memset(public_key_out, 0, POL_Ed25519_PK_SIZE);
        memset(secret_key_out, 0, POL_Ed25519_SK_SIZE);
    }
}
...
void signBeaconBroadcast(uint8_t signature_out[POL_SIG_SIZE], uint32_t beacon_id, uint64_t counter,
    ...
    if (crypto_sign_ed25519_detached(signature_out, &sig_len, signed_data_buffer, signed_data_len,
        ...
        memset(signature_out, 0, POL_SIG_SIZE);
    }
}
...
void generateX25519KeyPair(uint8_t public_key_out[POL_X25519_PK_SIZE],
    ...
    randombytes_buf(secret_key_out, POL_X25519_SK_SIZE);
    ...
    if (crypto_scalarmult_curve25519_base(public_key_out, secret_key_out) != 0) {
        ...
        memset(public_key_out, 0, POL_X25519_PK_SIZE);
...
bool deriveAEADSharedKey(uint8_t shared_key_out[POL_SHARED_KEY_SIZE],
    ...
    if (crypto_scalarmult_curve25519(shared_key_out, my_x25519_secret_key,
        ...
        memset(shared_key_out, 0, POL_SHARED_KEY_SIZE);
        return false;
    }
    return true;
}
